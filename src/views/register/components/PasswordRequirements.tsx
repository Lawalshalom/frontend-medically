...
const PasswordRequirements = ({ value = '' }: Props): JSX.Element => {
  return (
    <>
      <ConditionWrapper className={value?.length >= 8 ? 'text-primary' : ''}>
        <CheckCircleFilled />
        <Text>8 characters minimum</Text>
      </ConditionWrapper>
      <ConditionWrapper className={value?.match(/[A-Z]/g) ? 'text-primary' : ''}>
        <CheckCircleFilled />
      ...
      <ConditionWrapper className={value?.match(/\d/) ? 'text-primary' : ''}>
        <CheckCircleFilled />
